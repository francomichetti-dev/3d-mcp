...
2. ~~**[GUI]** Verify Fusion loads a **symlinked** add-in folder on this Mac~~ *(**confirmed 2026-08-05**: the probe add-in loaded from a symlink under `~/Library/Application Support/Autodesk/Autodesk Fusion 360/API/AddIns/` on Fusion **2704.1.36**. Autodesk's docs are silent on symlinks, so this needed proving; `install.sh`'s symlink mechanism is validated and no copy-mode fallback is needed. Probe since removed.)* **Note for anyone hitting "FusionBridge doesn't appear in the Add-Ins list": Fusion scans that folder only at launch — restart Fusion after installing.**
