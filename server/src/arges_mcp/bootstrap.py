...
the add-in. Re-run `arges install` after an upgrade, and the version
...
def run_install(rotate_token: bool = False) -> int:
    ...
    print(MANUAL_STEP)
    print("Then point your MCP client at:  arges")
    return 0
...
def run_status() -> int:
    ...
    if TOKEN_PATH.is_file():
        try:
            ...
            pass
    print(f"  token:  {'present' if token else 'MISSING — run: arges install'}")
