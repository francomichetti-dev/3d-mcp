...
- **File-based polling, not HTTP**: the MCP server writes `command_*.json` into `~/fusion_mcp_comm/`; the add-in's background daemon thread globs the directory every 0.1 s, executes, and writes `response_<command_id>.json` back. `run()` creates the comm dir and starts the monitor thread; `stop()` flips a `stop_thread` flag.
